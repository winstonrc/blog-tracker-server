...
const App = () => {
  ...
  const addBlog = async (blogObject) => {
    ...
    try {
      ...
      setNotificationColor('green')
      setNotification(`Added ${blogObject.title} by ${blogObject.author} to the list`)
      setTimeout(() => {
        setNotification(null)
      }, 5000)
    } catch (error) {
  ...
  return (
    <div>
      ...
      {user === null ?
        ...
        <div>
          <p>Logged in as {user.name} {<button className='logoutButton' onClick={handleLogout}>logout</button>}</p>
          {blogForm()}
          <br></br>
